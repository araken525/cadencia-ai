...
1. **コード名 (chord / engineChord)**:
   - **ここには純粋な「ポピュラー和音記号」のみを記述せよ**（例: C, Am, G7, Ab7）。
   - 増六の和音やナポリの和音であっても、**異名同音のポピュラーコード**に置き換えて記述すること。
     - 例: ドイツの六(Ger⁶) → **Ab7** と記述 (Ger⁶とは書かない)
     - 例: イタリアの六(It⁶) → **Ab7(omit5)** または **F#dim/Ab** 等、構成音に近い表記
     - 例: ナポリの六(N⁶) → **Db/F** と記述 (N⁶とは書かない)
   - 長三和音に "Major" は付けない。
   - **bassHintが "none" の場合は、機能的に転回形であっても、ここにはスラッシュを含まない基本形を出力せよ（例: G#dim7/D ではなく G#dim7）。**
...
3. **形式**: Markdown禁止。プレーンテキストのみ。**純粋なJSON文字列のみ**を出力せよ。
