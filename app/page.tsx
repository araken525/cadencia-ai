...
// --- Design Constants: Friendly & Clean ---
const G = {
  // 背景: 温かみのあるオフホワイト（和紙のような白）
  bgMain: "bg-[#FDFBF7]",
  
  // メインカラー: 親しみやすいインディゴブルー（グラデなし単色）
  primary: "bg-indigo-500 text-white shadow-md shadow-indigo-200",
  ...
  // ガラス風素材（キーボード等に使用）
  glass: "bg-white/80 backdrop-blur-xl border border-white/60 shadow-lg shadow-slate-200/50",
  
  // カードのスタイル（紙のような質感）
  card: "bg-white border border-slate-100 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] rounded-[24px]",
  
  // アクセント（状態表示）
  accentRoot: "bg-rose-100 text-rose-600 border-rose-200",
  accentBass: "bg-amber-100 text-amber-600 border-amber-200",
};
         ...
         {keys.filter(k => k.type === "white").map((k) => (
           <path key={k.idx} d={`M${k.x},0 h14.28 v44 a4,4 0 0 1 -4,4 h-6.28 a4,4 0 0 1 -4,-4 z`}
             className={`transition-all duration-200 ${
               isActive(k.idx) 
                 ? (isRoot(k.idx) ? "fill-rose-300 stroke-rose-400" : isBass(k.idx) ? "fill-amber-300 stroke-amber-400" : "fill-indigo-300 stroke-indigo-400") 
...
const FlickKey = ({ 
  ...
  return (
    <div className={`
      ...
      ${isRoot ? "bg-rose-50 border-2 border-rose-200" 
        : isBass ? "bg-amber-50 border-2 border-amber-200" 
        : "bg-white/60 border border-white/50 hover:bg-white/80"}
      ${!isBass && !isRoot && isActive ? "bg-indigo-50 border-2 border-indigo-200" : ""}
    ...
    onPointerDown={handlePointerDown} onPointerMove={handlePointerMove} onPointerUp={handlePointerUp} onPointerCancel={handlePointerUp}>
      ...
      {/* Guide Indicators */}
      <div className={`absolute top-1.5 left-0 right-0 flex justify-center transition-all ${isUp ? "opacity-100 -translate-y-1 text-indigo-500" : "opacity-0"}`}>
        <span className="text-[10px] font-black">♯</span>
      </div>
      <div className={`absolute bottom-1.5 left-0 right-0 flex justify-center transition-all ${isDown ? "opacity-100 translate-y-1 text-indigo-500" : "opacity-0"}`}>
        <span className="text-[10px] font-black">♭</span>
      </div>

      {/* Status Dots (Cute Style) */}
      {isRoot && <div className="absolute top-2 right-2 w-2 h-2 rounded-full bg-rose-400 ring-2 ring-white" />}
      {isBass && <div className="absolute top-2 right-2 w-2 h-2 rounded-full bg-amber-400 ring-2 ring-white" />}
      
      {/* Label */}
      <span className={`text-2xl font-bold tracking-tight transition-transform ${isRoot ? "text-rose-500" : isBass ? "text-amber-600" : "text-slate-600"} ${isActive && !isRoot && !isBass ? "text-indigo-600" : ""}`} 
...
// 3. Result Card (Clean Paper Style)
const ResultCard = ({ candidate, isTop, isKeySet }: { candidate: CandidateObj, isTop: boolean, isKeySet: boolean }) => {
  ...
  return (
    <div className={`relative overflow-hidden transition-all duration-500 group animate-in slide-in-from-bottom-4 fade-in
      ${isTop 
        ? "bg-white border-2 border-indigo-100 shadow-xl shadow-indigo-100/50 rounded-[28px] p-6" 
        : "bg-white border border-slate-100 shadow-sm rounded-2xl p-4"}
    `}>
      {/* Rank Number (Subtle) */}
      <div className={`absolute -right-2 -bottom-6 font-black text-slate-100 select-none z-0 pointer-events-none transform -rotate-12 ${isTop ? "text-9xl" : "text-7xl"}`}>
        ...
        <div className="flex justify-between items-start">
          <div className="flex flex-col gap-1.5">
            <div className="flex flex-wrap gap-2 items-center">
              {isTop && (
                <span className={`px-2.5 py-0.5 rounded-full text-[11px] font-bold tracking-wide border ${isProvisional ? "bg-amber-50 text-amber-600 border-amber-200" : "bg-indigo-50 text-indigo-600 border-indigo-200"}`}>
          ...
          <div className="flex flex-col items-end">
            ...
            <span className={`font-black ${isTop ? "text-3xl text-indigo-500" : "text-sm text-indigo-400"}`}>{percent}<span className="text-xs opacity-50">%</span></span>
          </div>
        </div>

        {/* Function Analysis Grid */}
        {isKeySet ? (
          <div className="bg-slate-50/70 rounded-2xl p-2 border border-slate-100 grid grid-cols-12 gap-2">
            {/* Function (TDS) */}
...
// 4. Insight Card (Friendly AI)
const InsightCard = ({ text }: { text: string }) => (
  <div className={`relative rounded-[24px] overflow-hidden ${G.card} p-6`}>
    <div className="flex items-center gap-3 mb-3">
      <div className="w-10 h-10 rounded-full bg-indigo-50 text-indigo-500 flex items-center justify-center border border-indigo-100 text-lg">
         🤖
      </div>
...
const AskCard = ({ question, setQuestion, ask, isThinking, loading, inputRefProp }: any) => (
  <div className={`relative rounded-[24px] overflow-hidden bg-gradient-to-br from-indigo-50 to-white border border-indigo-100 p-6 shadow-sm`}>
    <h3 className="text-sm font-bold text-indigo-900 mb-4 flex items-center gap-2">
      <span className="text-xl">🙋‍♂️</span> AIに質問してみる
    </h3>
    <div className="relative group">
      <input 
        ref={inputRefProp}
        className="w-full bg-white border border-indigo-100 rounded-2xl py-4 pl-5 pr-14 text-base focus:outline-none focus:ring-2 focus:ring-indigo-300 focus:border-transparent transition-all shadow-sm placeholder:text-slate-300 text-slate-700" 
        placeholder="例：なんでこの機能になるの？" 
        value={question} 
        onChange={(e) => setQuestion(e.target.value)} 
        onKeyDown={(e) => e.key === 'Enter' && ask()} 
        disabled={isThinking} 
      />
      <button onClick={ask} disabled={loading || isThinking || !question.trim()} className={`absolute right-2 top-1/2 -translate-y-1/2 p-2.5 rounded-xl text-white transition-all active:scale-90 shadow-md ${!question.trim() ? "bg-slate-200 text-slate-400" : "bg-indigo-500 hover:bg-indigo-600"}`}>
...
const LoadingOverlay = () => (
  <div className="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-white/80 backdrop-blur-md animate-in fade-in duration-300">
    <div className="relative w-24 h-24 bg-white rounded-3xl shadow-xl flex items-center justify-center border border-indigo-50 animate-bounce">
       <span className="text-4xl">🎹</span>
    </div>
    <div className="mt-6 text-center space-y-1">
      <h2 className="text-lg font-bold text-slate-700">考え中... 💭</h2>
      <p className="text-xs font-medium text-slate-400">AIが音楽理論を分析しています</p>
    </div>
  </div>
);
    ...
    <div className={`min-h-screen ${G.bgMain} ${G.textMain} font-sans pb-[450px] selection:bg-indigo-100 overflow-x-hidden`}>
      ...
      <header className={`fixed top-0 inset-x-0 z-50 h-16 bg-white/90 backdrop-blur-md border-b border-slate-100 flex items-center justify-between px-5 transition-all`}>
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-indigo-500 flex items-center justify-center text-white shadow-sm text-lg">
             🎹
          </div>
          <div className="flex flex-col justify-center leading-none">
            <span className="text-[10px] font-bold text-indigo-500 tracking-wide mb-0.5">音楽理論AIアシスタント</span>
            <div className="flex items-center gap-2">
              <span className={`text-lg font-black tracking-tight text-slate-800`}>Cadencia AI</span>
              <FeedbackLink className="bg-indigo-50 border border-indigo-100 text-[9px] font-bold text-indigo-600 px-2 py-0.5 rounded-full hover:bg-indigo-100 transition-colors flex items-center gap-1">
                <span>BETA 🐣</span>
              </FeedbackLink>
            </div>
          </div>
        ...
        <section className={`${G.card} p-5 transition-all duration-300 ${justUpdated ? "ring-2 ring-indigo-100" : ""}`}>
           <div className="flex justify-between items-center mb-4">
              <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-2">
                <IconKeyboard className="w-4 h-4" /> 音を入力してね
              </h3>
              <span className="text-[10px] font-bold text-indigo-500 bg-indigo-50 px-2 py-1 rounded-full">{selected.length} 音 選択中</span>
           </div>
      ...
      <div className={`fixed bottom-0 inset-x-0 z-50 ${G.glass} rounded-t-[32px] pt-5 pb-8 shadow-[0_-10px_40px_rgba(0,0,0,0.05)] transition-transform duration-300`}>
        <div className="max-w-md mx-auto px-4">
          <div className="grid grid-cols-4 grid-rows-4 gap-2 h-full">
            ...
            <div className="col-start-1 row-start-3 h-14 flex flex-col gap-1">
               <button onClick={() => setInputMode(m => m === "root" ? "normal" : "root")} className={`flex-1 rounded-xl text-[10px] font-bold transition-all border-b-2 ${inputMode === "root" ? "bg-rose-500 text-white border-rose-600 shadow-sm" : "bg-white text-slate-500 border-slate-200"}`}>根音</button>
               <button onClick={() => setInputMode(m => m === "bass" ? "normal" : "bass")} className={`flex-1 rounded-xl text-[10px] font-bold transition-all border-b-2 ${inputMode === "bass" ? "bg-amber-500 text-white border-amber-600 shadow-sm" : "bg-white text-slate-500 border-slate-200"}`}>最低音</button>
            </div>
            ...
            <button className={`col-start-4 row-start-3 row-span-2 rounded-2xl flex flex-col items-center justify-center shadow-lg transition-all active:scale-95 border-b-4 active:border-b-0 active:translate-y-1 ${canAnalyze && !loading ? "bg-indigo-500 border-indigo-700 text-white" : "bg-slate-200 border-slate-300 text-slate-400 cursor-not-allowed"}`} onClick={analyze} disabled={!canAnalyze || loading}>
