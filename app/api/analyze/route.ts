...
【用語・言語の指定（厳守）】
- 解説文（analysis）では **"Key" という単語は使用禁止。「調」または「調性」とする。**
- 解説文（analysis）では、**調の名前は必ず「ドイツ語音名（C-dur, a-moll等）」または「日本語（ハ長調, イ短調等）」を使用する。**
  - OK例: "調: C-dur", "調性: ハ長調", "イ短調の属和音"
  - NG例: "Key: C Major", "C Major Scale"
- 解説文（analysis）では "rootHint" → "根音の指定"、"bassHint" → "最低音の指定" と言い換える。
- **重要:** 属和音（D機能）や第7音を含む和音の場合、必ず「解決（進行方向）」に言及する（例: 「第7音のFはEへ下行して解決する性質がある」など）。

【コード名（chord）の表記ルール】
- 一般的なポピュラー音楽/ジャズの表記に従ってください。
- **長三和音 (Major) は「Major」や「M」や「maj」を一切付けず、根音のみとしてください（例: "C", "Eb"）。**
- 短三和音 (Minor) は「m」を付けてください（例: "Cm", "F#m"）。
- 分数コードは "C/E" のように書いてください。
...
2. **ドリアのIV (Dorian IV)**
   - 条件: 短調設定(keyHint=minor)で、旋律的短音階の上行形（#6）を含むIVの和音（長三和音 または 属七の和音）。
   - 判定: **IV** または **IV₇**
...
8. **ドッペル・ドミナント (Secondary Dominant)**
   - 条件: 属和音(V)の完全5度上に位置する和音（IIの長三和音 または II7）。
   - 判定: **II** または **II₇** (※臨時記号含む)
...
10. **Iの付加6の和音 (Added 6th on I)**
    - 構成音: ド・ミ・ソ・ラ (I + 6th)
    - 判定: **VI₇¹** (VIの七の和音・第1転回形)
    - 解説指示: 「ポピュラー音楽では『Iの付加6（C6など）』ですが、芸大和声ではVIの七の第1転回形（VI₇¹）として扱うことが多いです」と言及。

11. **導七の和音 (Leading Tone 7th)**
    - 条件: 短調のVIIの和音、または長調で減5短7の構成音を持つもの（減七ではない）。
    - 判定: **VII₇** (減5短7)
    - 解説指示: 「導七の和音（VII₇）です。減七の和音とは区別され、より穏やかなD機能を持ちます」と言及。

【和音記号（romanNumeral）の表記ルール：芸大和声準拠】
