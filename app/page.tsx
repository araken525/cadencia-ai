...
// --- Design Constants: Design 3 (Neo Vintage / Magic Style) ---
const G = {
  // 背景: 古びた羊皮紙（エイジド・パーチメント）
  bgMain: "bg-[#F2EFE5]",
  
  // テキスト: 魔法使いのインク（セピアがかった深い黒）
  ...
  accentMagic: "text-teal-800 bg-teal-50 border-teal-200 shadow-[0_0_15px_rgba(20,184,166,0.3)]",
  
  // マテリアル: 魔法がかけられたルーン石板（キーボード用）
  stonePlate: "bg-[#E8E4D9] border-2 border-[#C5B498] shadow-[3px_3px_0px_#B0A080] active:translate-y-[2px] active:shadow-[1px_1px_0px_#B0A080] transition-all",
  
  // マテリアル: 古文書のページ（カード用）
  parchmentCard: "bg-[#FDFBF7] border-2 border-[#DBCAB0] shadow-md shadow-[#2A211C]/5 rounded-lg",
  
  // 魔法の輝き（選択状態やトップ評価）
  magicGlow: "shadow-[0_0_20px_rgba(180,142,67,0.6)] border-[#B48E43] z-10 relative animate-pulse-slow",
};
...
const MiniPiano = ({ selected, bassHint, rootHint }: { selected: string[], bassHint: string | null, rootHint: string | null }) => {
  ...
  return (
    <div className="h-16 w-full relative select-none pointer-events-none p-1.5 bg-[#2A211C] rounded-sm shadow-inner border border-[#B48E43]">
         ...
         {keys.filter(k => k.type === "white").map((k) => (
           <path key={k.idx} d={`M${k.x},0 h14.28 v44 a2,2 0 0 1 -2,2 h-10.28 a2,2 0 0 1 -2,-2 z`}
             className={`transition-all duration-300 ${
               isActive(k.idx) 
                 ? (isRoot(k.idx) ? "fill-rose-300" : isBass(k.idx) ? "fill-amber-300" : "fill-teal-200") 
                 : "fill-[#FDFBF7]"
...
const FlickKey = ({ 
  ...
  return (
    ...
    onPointerDown={handlePointerDown} onPointerMove={handlePointerMove} onPointerUp={handlePointerUp} onPointerCancel={handlePointerUp}>
      ...
      {/* Label */}
      <span className={`text-2xl font-black tracking-tight drop-shadow-sm ${isRoot ? "text-rose-800" : isBass ? "text-amber-800" : "text-[#2A211C]"} ${isActive && !isRoot && !isBass ? "text-teal-900" : ""}`} 
...
// 3. Result Card (Parchment Scroll)
      ...
      <div className="relative z-10 flex flex-col gap-4">
        ...
        <div className="flex justify-between items-start">
          ...
          <div className="flex flex-col items-end">
            <span className="text-[10px] text-[#8C7B70] font-bold font-serif uppercase tracking-widest mb-0.5">信頼度</span>
      ...
      <main className="pt-24 px-5 max-w-md mx-auto space-y-6 relative z-10">
        ...
        <section className={`${G.parchmentCard} p-5 transition-all duration-300 relative ${justUpdated ? G.magicGlow : ""}`}>
           ...
           <div className="flex justify-between items-center mb-4 relative z-10">
              ...
              <span className="text-[10px] font-serif font-bold text-[#3E3229] bg-[#E8E4D9] px-2 py-1 rounded-sm border border-[#C5B498] shadow-sm">{selected.length} 音</span>
           </div>
        ...
        {hasResult && (
          <div ref={resultRef} className="space-y-6 animate-in fade-in slide-in-from-bottom-8 duration-700">
             ...
             <div className="flex items-center gap-3 px-1">
               ...
               <h2 className="text-sm font-serif font-black text-[#3E3229] uppercase tracking-widest">分析結果</h2>
