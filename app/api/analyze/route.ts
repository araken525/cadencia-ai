...
8. [IVの付加6] (IV + 6th)
   ...
   - 判定: II₇ (※転回形の指定はせず、単にII₇とする)
   - 解説: 「機能的にはVへ進むII₇(またはIVの付加6)」と言及。

9. [Iの付加6] (I + 6th)
    - 条件: Iの和音に第6音が付加された形 (例: C調で C, E, G, A)。
    - 判定: VI₇ (※転回形の指定はせず、単にVI₇とする)
    - 解説: 「芸大和声ではVIの七として扱う(またはIの付加6)」と言及。
...
// 【追加】難解な和音のためのカンニングペーパー (増六・付加6)
const COMPLEX_CHORD_EXAMPLES = `
【判定サンプル（迷ったらこれに従え）】
// 増六の和音 (Key: C の例)
入力: [Ab, C, F#] → 正解: "It⁶" (イタリアの六)
入力: [Ab, C, Eb, F#] → 正解: "Ger⁶" (ドイツの六)
入力: [Ab, C, D, F#] → 正解: "Fr⁶" (フランスの六)

// 付加6の和音 (Key: C の例)
入力: [F, A, C, D] (IV+6)
→ 正解: "II₇"
→ 解説: "IVの付加6だが、芸大和声では機能的にII₇として扱う。"

入力: [C, E, G, A] (I+6)
→ 正解: "VI₇"
→ 解説: "Iの付加6だが、芸大和声では機能的にVI₇として扱う。"

※他の調でも音程関係が同じなら同様に判定せよ。
`;
