...
// 1. イントロダクション (Dark Mode)
const WelcomeModal = ({ onClose }: { onClose: () => void }) => {
  const [isClosing, setIsClosing] = useState(false);
  const handleClose = () => { setIsClosing(true); setTimeout(onClose, 300); };

  return (
    <div className={`fixed inset-0 z-[100] bg-black/80 backdrop-blur-md flex items-center justify-center p-4 transition-opacity duration-300 ${isClosing ? "opacity-0" : "opacity-100"}`}>
      <div className={`w-full max-w-md h-[85vh] bg-slate-900 rounded-[40px] shadow-2xl border border-slate-800 overflow-hidden relative transform transition-all duration-300 flex flex-col ${isClosing ? "scale-95 translate-y-8 opacity-0" : "scale-100 translate-y-0 opacity-100"}`}>
        <div className="absolute top-10 -left-10 text-[8rem] font-black text-slate-800 rotate-90 pointer-events-none select-none opacity-50">INTRODUCTION</div>
        <div className="flex-1 overflow-y-auto p-8 relative z-10 scrollbar-hide">
          <div className="text-center mb-10">
            <div className="w-16 h-16 rounded-2xl bg-slate-800 flex items-center justify-center text-white text-3xl shadow-xl mx-auto mb-6 rotate-3 border border-slate-700">🎹</div>
            <h1 className="text-4xl font-black text-slate-100 tracking-tighter mb-2">Waon AI</h1>
            <p className="text-sm font-bold text-slate-400">ポケットに、専属の音楽理論家を。</p>
          </div>
          
          <div className="mb-10">
            <h2 className="text-sm font-black text-slate-100 border-b-2 border-slate-800 pb-2 mb-4 flex items-center gap-2"><span className="text-xl">🎯</span> 対象ユーザー</h2>
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-slate-800/50 p-4 rounded-3xl border border-slate-700/50">
                <h3 className="font-bold text-slate-200 mb-2 text-xs flex items-center gap-1.5"><span className="text-base">🎺</span> 奏者の方へ</h3>
                <ul className="space-y-1.5 text-[10px] text-slate-400 font-medium leading-tight list-disc list-outside pl-3">
                  <li>和音を言語化したい</li>
                  <li>スコアの理解を深めたい</li>
                  <li>音楽的意味を知りたい</li>
                </ul>
              ...
              <div className="bg-slate-800/50 p-4 rounded-3xl border border-slate-700/50">
                <h3 className="font-bold text-slate-200 mb-2 text-xs flex items-center gap-1.5"><span className="text-base">🎓</span> 学ぶ方へ</h3>
                <ul className="space-y-1.5 text-[10px] text-slate-400 font-medium leading-tight list-disc list-outside pl-3">
                  <li>和声用語で解説が欲しい</li>
                  <li>転回形やバスを学びたい</li>
                  <li>自習の答え合わせに</li>
          ...
          <div className="mb-8">
            <h2 className="text-sm font-black text-slate-100 border-b-2 border-slate-800 pb-2 mb-4 flex items-center gap-2"><span className="text-xl">✨</span> Waon AIの特徴</h2>
            <div className="text-xs text-slate-400 leading-relaxed font-medium space-y-4">
              <p>入力された構成音から和音を判定し、その音楽的意味を<span className="bg-yellow-500/20 text-yellow-200 font-bold px-1 rounded">「和声学の言葉」</span>で解説する音楽理論特化型AI解析アプリです。</p>
              
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-blue-900/20 border border-blue-900/30 p-3 rounded-2xl text-center"><div className="text-lg mb-1">🧐</div><div className="font-bold text-blue-400">根拠</div><div className="text-[9px] text-blue-300">なぜその和音か</div></div>
                <div className="bg-rose-900/20 border border-rose-900/30 p-3 rounded-2xl text-center"><div className="text-lg mb-1">⚙️</div><div className="font-bold text-rose-400">機能</div><div className="text-[9px] text-rose-300">調性内の役割</div></div>
                <div className="bg-emerald-900/20 border border-emerald-900/30 p-3 rounded-2xl text-center"><div className="text-lg mb-1">🏗️</div><div className="font-bold text-emerald-400">構造</div><div className="text-[9px] text-emerald-300">転回形・バス</div></div>
                <div className="bg-purple-900/20 border border-purple-900/30 p-3 rounded-2xl text-center"><div className="text-lg mb-1">💡</div><div className="font-bold text-purple-400">多義性</div><div className="text-[9px] text-purple-300">他の解釈</div></div>
              </div>

              {/* Added: モード切り替え説明 */}
              <div className="mt-2 bg-slate-800/50 border border-slate-700 p-3 rounded-2xl flex items-center gap-3">
                 <div className="flex -space-x-1 shrink-0">
                    <div className="w-6 h-6 rounded-full bg-slate-700 border border-emerald-900/50 flex items-center justify-center text-xs shadow-sm z-10">🔰</div>
                    <div className="w-6 h-6 rounded-full bg-slate-700 border border-blue-900/50 flex items-center justify-center text-xs shadow-sm">🎓</div>
                 </div>
                 <div>
                    <h4 className="font-bold text-slate-200 text-[10px]">選べる2つのモード</h4>
              ...
              <p className="text-center font-bold text-slate-600 mt-2">プロの音楽家の思考プロセスを、AIが可視化します。</p>
            </div>
          </div>

          <div className="mb-8">
            <h2 className="text-sm font-black text-slate-100 border-b-2 border-slate-800 pb-2 mb-4 flex items-center gap-2"><span className="text-xl">🎹</span> キーボード操作</h2>
            <div className="bg-gradient-to-br from-slate-800 to-blue-900/20 rounded-3xl p-5 border border-slate-700">
               {/* Updated: Grid to accommodate new flick actions */}
               <div className="grid grid-cols-2 gap-4">
                  <div className="flex items-center gap-2"><span className="w-8 h-8 rounded-lg bg-slate-700 shadow-sm flex items-center justify-center text-xs border border-slate-600">👆</span><div className="text-[10px] font-bold text-slate-300 leading-tight">タップ<br/><span className="text-slate-500 font-normal">音を入力</span></div></div>
                  
                  {/* Root/Bass */}
                  <div className="flex items-center gap-2"><span className="w-8 h-8 rounded-lg bg-rose-900/20 shadow-sm flex items-center justify-center text-xs border border-rose-900/30 text-rose-500">R</span><div className="text-[10px] font-bold text-slate-300 leading-tight">根音指定<br/><span className="text-rose-400 font-bold">Root</span></div></div>
                  <div className="flex items-center gap-2"><span className="w-8 h-8 rounded-lg bg-amber-900/20 shadow-sm flex items-center justify-center text-xs border border-amber-900/30 text-amber-500">B</span><div className="text-[10px] font-bold text-slate-300 leading-tight">最低音指定<br/><span className="text-amber-400 font-bold">Bass</span></div></div>
                  <div className="flex items-center gap-2"><span className="w-8 h-8 rounded-lg bg-slate-700 shadow-sm flex items-center justify-center text-xs border border-slate-600 text-purple-400">🗝</span><div className="text-[10px] font-bold text-slate-300 leading-tight">調性指定<br/><span className="text-purple-400 font-bold">Key</span></div></div>

                  {/* Flicks */}
                  <div className="flex items-center gap-2"><span className="w-8 h-8 rounded-lg bg-slate-700 shadow-sm flex items-center justify-center text-xs border border-slate-600 text-blue-400">⬆️</span><div className="text-[10px] font-bold text-slate-300 leading-tight">上フリック<br/><span className="text-blue-400 font-bold"># シャープ</span></div></div>
                  <div className="flex items-center gap-2"><span className="w-8 h-8 rounded-lg bg-slate-700 shadow-sm flex items-center justify-center text-xs border border-slate-600 text-blue-400">⬇️</span><div className="text-[10px] font-bold text-slate-300 leading-tight">下フリック<br/><span className="text-blue-400 font-bold">b フラット</span></div></div>
                  
                  {/* Added: Double Accidentals */}
                  <div className="flex items-center gap-2"><span className="w-8 h-8 rounded-lg bg-slate-700 shadow-sm flex items-center justify-center text-xs border border-slate-600 text-blue-400">➡️</span><div className="text-[10px] font-bold text-slate-300 leading-tight">右フリック<br/><span className="text-blue-400 font-bold">## ダブルシャープ</span></div></div>
                  <div className="flex items-center gap-2"><span className="w-8 h-8 rounded-lg bg-slate-700 shadow-sm flex items-center justify-center text-xs border border-slate-600 text-blue-400">⬅️</span><div className="text-[10px] font-bold text-slate-300 leading-tight">左フリック<br/><span className="text-blue-400 font-bold">bb ダブルフラット</span></div></div>
               </div>
               <p className="mt-4 text-[9px] text-slate-500 text-center">直感的なフリック操作で、素早く音符を入力できます。</p>
            </div>
          </div>

          <div className="mb-4">
            <h2 className="text-sm font-black text-slate-100 border-b-2 border-slate-800 pb-2 mb-4 flex items-center gap-2"><span className="text-xl">📚</span> 理論と表記の基準</h2>
            {/* Updated: Content completely rewritten */}
            <div className="bg-slate-800/50 rounded-3xl p-5 border border-slate-700 space-y-4">
               ...
               <div className="flex gap-3 items-start">
                  <div className="w-6 h-6 rounded-lg bg-slate-700 border border-slate-600 shadow-sm flex items-center justify-center shrink-0 text-xs">📖</div>
                  <div>
                     <h4 className="text-[10px] font-bold text-slate-200 mb-0.5">特殊和音の判定辞書</h4>
               ...
               <div className="flex gap-3 items-start">
                  <div className="w-6 h-6 rounded-lg bg-slate-700 border border-slate-600 shadow-sm flex items-center justify-center shrink-0 text-xs">🏛</div>
                  <div>
                     <h4 className="text-[10px] font-bold text-slate-200 mb-0.5">記号表記について</h4>
                     <p className="text-[9px] text-slate-400 leading-relaxed">
        ...
        <div className="p-6 bg-slate-900 border-t border-slate-800 relative z-20">
          <button onClick={handleClose} className="w-full py-4 rounded-2xl bg-slate-100 text-slate-900 font-bold shadow-lg hover:bg-white hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-2 group"><span>はじめる</span><span className="group-hover:translate-x-1 transition-transform">→</span></button>
        </div>
      </div>
    </div>
  );
...
const FlickKey = ({ 
  ...
  return (
    <div className={`relative rounded-2xl touch-none select-none overflow-visible flex flex-col items-center justify-center transition-all duration-100 z-0 ${isRoot ? "bg-rose-900/30 border border-rose-800 shadow-[0_0_12px_rgba(244,63,94,0.3)]" : isBass ? "bg-amber-900/30 border border-amber-800 shadow-[0_0_12px_rgba(251,191,36,0.3)]" : G.glassKey} ${!isBass && !isRoot && isActive ? "bg-cyan-900/30 border-cyan-800 shadow-[0_0_12px_rgba(34,211,238,0.3)]" : ""} ${className}`} onPointerDown={handlePointerDown} onPointerMove={handlePointerMove} onPointerUp={handlePointerUp} onPointerCancel={handlePointerUp}>
      <div className={`absolute top-1 left-0 right-0 flex justify-center transition-all duration-150 pointer-events-none ${isUp ? "opacity-100 scale-125 text-blue-400 font-black translate-y-[-2px]" : "opacity-30 text-slate-500"}`}><span className="text-[10px] leading-none">♯</span></div>
      <div className={`absolute bottom-1 left-0 right-0 flex justify-center transition-all duration-150 pointer-events-none ${isDown ? "opacity-100 scale-125 text-blue-400 font-black translate-y-[2px]" : "opacity-30 text-slate-500"}`}><span className="text-[10px] leading-none">♭</span></div>
      <div className={`absolute left-1 top-0 bottom-0 flex items-center transition-all duration-150 pointer-events-none ${isLeft ? "opacity-100 scale-125 text-blue-400 font-black translate-x-[-2px]" : "opacity-30 text-slate-500"}`}><span className="text-[9px] leading-none">𝄫</span></div>
      <div className={`absolute right-1 top-0 bottom-0 flex items-center transition-all duration-150 pointer-events-none ${isRight ? "opacity-100 scale-125 text-blue-400 font-black translate-x-[2px]" : "opacity-30 text-slate-500"}`}><span className="text-[9px] leading-none">𝄪</span></div>
      <span className={`text-2xl font-medium tracking-tight transition-all duration-100 ${isRoot ? "text-rose-400" : isBass ? "text-amber-400" : isActive ? "text-cyan-400" : "text-slate-200"}`}>{displayLabel}</span>
    </div>
  );
};
...
const ResultCard = ({ candidate, isTop, isKeySet, rank }: { candidate: CandidateObj, isTop: boolean, isKeySet: boolean, rank: number }) => {
  ...
  return (
    <div className={`relative overflow-hidden transition-all duration-700 group ${G.cardBase} p-0`}>
      <div className={`absolute -right-4 -bottom-6 font-black select-none pointer-events-none z-0 tracking-tighter leading-none ${isTop ? "text-slate-800 text-[10rem]" : "text-slate-800 text-[6rem]"}`}>{String(rank).padStart(2, '0')}</div>
      <div className="relative z-10 p-6 flex flex-col gap-6">
        <div className="flex justify-between items-start">
          <div className="flex flex-col gap-2">
             {isTop && (
               <div className="flex items-center gap-2">
                 <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-bold border shadow-sm ${isProvisional ? "bg-amber-900/30 text-amber-400 border-amber-800" : "bg-amber-900/20 text-amber-400 border-amber-800"}`}>
                   {isProvisional ? "⚠️ 暫定判定" : "👑 最有力候補"}
                 </span>
               </div>
             )}
             <h2 className="text-5xl font-black text-slate-100 tracking-tighter leading-none">{formatNote(candidate.chord)}</h2>
          </div>
          <div className="text-right">
             <div className="flex items-baseline justify-end gap-1">
               <span className="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-br from-blue-400 to-cyan-300">{percent}</span>
        ...
        <div className="bg-slate-950 rounded-2xl p-4 border border-slate-800 shadow-inner flex items-stretch justify-between divide-x divide-slate-800 h-24">
            ...
            <div className={`flex-1 flex flex-col items-center justify-center px-1`}>
                <span className="text-[9px] font-bold text-slate-500 mb-1">記号</span>
                <span className={`text-xl font-serif font-black leading-none ${!isKeySet ? "text-slate-600" : "text-slate-200"}`}>{!isKeySet ? "―" : (candidate.romanNumeral || "―")}</span>
            </div>
            <div className="flex-1 flex flex-col items-center justify-center px-1">
                <span className="text-[9px] font-bold text-slate-500 mb-1">転回形</span>
                <span className="text-xs font-bold text-slate-300 leading-none text-center">{invJp}</span>
            </div>
            <div className="flex-1 flex flex-col items-center justify-center px-1">
                <span className="text-[9px] font-bold text-slate-500 mb-1">種類</span>
                <span className="text-xs font-bold text-slate-300 leading-none text-center">{candidate.chordType || "―"}</span>
            </div>
        </div>
      </div>
    </div>
...
const InsightCard = ({ text, onAsk }: { text: string, onAsk: () => void }) => (
  <div className={`${G.cardBase} p-6 overflow-hidden bg-gradient-to-br from-slate-800 to-slate-900`}>
    <div className="absolute -right-4 top-2 text-[5rem] font-black text-white/5 pointer-events-none select-none z-0 transform rotate-[-5deg] tracking-tighter leading-none whitespace-nowrap">Waon AI</div>
    <div className="relative z-10">
      <div className="flex items-center gap-3 mb-4">
        <div className="w-8 h-8 rounded-full flex items-center justify-center text-white bg-gradient-to-br from-blue-600 to-cyan-600 shadow-md"><IconBook className="w-4 h-4" /></div>
        <h3 className="text-sm font-bold text-slate-200">Waon AI の考察</h3>
      </div>
      <p className="text-sm leading-relaxed text-slate-300 whitespace-pre-wrap font-medium mb-6">{text}</p>
      
      <button 
        onClick={onAsk}
        className="w-full py-3 rounded-2xl border border-slate-700 bg-slate-800 text-slate-300 text-xs font-bold shadow-sm hover:bg-slate-700 hover:text-white transition-all flex items-center justify-center gap-2 active:scale-95"
      >
  ...
  return (
    <div className={G.chatContainer}>
      <div className="flex flex-col h-[500px]">
        <div className="px-6 py-4 bg-slate-900/50 backdrop-blur-md border-b border-slate-700/50 flex justify-between items-center z-10">
          <div className="flex items-center gap-3"><div className="w-2.5 h-2.5 rounded-full bg-green-400 animate-pulse"></div><h3 className="font-bold text-slate-200">Waon AIのチャット</h3></div><IconRobot className="text-slate-500 w-5 h-5" />
        </div>
        <div ref={scrollRef} className="flex-1 overflow-y-auto p-5 space-y-6 scrollbar-hide">
          {history.length === 0 && !isThinking && (
            <div className="h-full flex flex-col items-center justify-center text-center space-y-4 opacity-50"><div className="w-16 h-16 bg-slate-800 rounded-3xl flex items-center justify-center shadow-lg border border-slate-700"><span className="text-3xl">💬</span></div><p className="text-xs font-bold text-slate-500">知りたいことを入力して<br/>AIと対話しよう</p></div>
          )}
          {history.map((msg, i) => (
            <div key={i} className={`flex items-end gap-2 animate-in fade-in slide-in-from-bottom-2 duration-300 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
              {msg.role === 'ai' && <div className="w-8 h-8 rounded-full bg-slate-800 flex items-center justify-center shadow-sm shrink-0 border border-slate-700"><IconRobot className="w-4 h-4 text-blue-400" /></div>}
          ...
          {isThinking && (
            <div className="flex items-end gap-2 animate-in fade-in">
              <div className="w-8 h-8 rounded-full bg-slate-800 flex items-center justify-center shadow-sm shrink-0 border border-slate-700"><IconRobot className="w-4 h-4 text-blue-400 animate-bounce" /></div>
        ...
        <div className="p-4 bg-slate-900/60 backdrop-blur-md border-t border-slate-700/50">
          <div className="flex gap-2 overflow-x-auto no-scrollbar pb-3 mask-linear-fade">
            {SHORTCUT_QUESTIONS.map((q) => (
              <button key={q} onClick={() => { setQuestion(q); setTimeout(ask, 0); }} disabled={loading || isThinking} className="whitespace-nowrap text-[10px] font-bold text-slate-300 bg-slate-800/80 hover:bg-slate-700 border border-slate-600 hover:border-blue-400/50 px-3 py-1.5 rounded-full shadow-sm transition-all active:scale-95 shrink-0">{q}</button>
            ))}
          </div>
          <div className="relative flex items-center gap-2 bg-slate-800 rounded-2xl p-1.5 shadow-sm border border-slate-700 transition-shadow focus-within:shadow-md focus-within:ring-2 focus-within:ring-blue-900">
            <textarea ref={inputRefProp} className="flex-1 bg-transparent border-none rounded-xl py-3 px-3 text-base text-slate-200 placeholder:text-slate-500 focus:outline-none focus:ring-0 resize-none max-h-24 min-h-[48px] leading-relaxed" placeholder="質問を入力..." value={question} rows={1} onChange={(e) => setQuestion(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); ask(); } }} disabled={isThinking} />
            <button onClick={ask} disabled={loading || isThinking || !question.trim()} className={`w-10 h-10 rounded-xl flex items-center justify-center text-white shrink-0 transition-all ${!question.trim() ? "bg-slate-700 text-slate-500 cursor-default" : "bg-blue-600 hover:bg-blue-500 shadow-lg shadow-blue-500/30 active:scale-90"}`}><IconSend className="w-4 h-4" /></button>
          </div>
        </div>
      </div>
    </div>
...
// 修正: 「AIの思考」を可視化する新しいローディング画面 (Dark Mode)
const LoadingOverlay = () => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-slate-950/90 backdrop-blur-xl animate-in fade-in duration-500 transition-all">
      {/* Animated Icon Container */}
      <div className="relative mb-10">
        {/* 波紋アニメーション (Ping) */}
        <div className="absolute inset-0 bg-blue-500/30 rounded-full animate-ping opacity-20 duration-1000"></div>
        <div className="absolute inset-[-12px] bg-gradient-to-tr from-cyan-500/20 to-blue-500/20 rounded-full blur-xl animate-pulse opacity-60"></div>
        
        <div className="relative w-28 h-28 bg-slate-900 rounded-[32px] shadow-[0_20px_50px_-12px_rgba(59,130,246,0.2)] flex items-center justify-center border border-slate-700 ring-4 ring-blue-900/30">
          {/* Bouncing Robot */}
          <div className="animate-bounce duration-[2000ms]">
             <IconRobot className="w-12 h-12 text-slate-200" />
          </div>
        </div>
      </div>
      ...
      <div className="text-center space-y-4 px-8 relative z-10 max-w-xs">
        <h2 className="text-xl font-black text-slate-100 tracking-tight flex items-center justify-center gap-2">
          <span>Waon AI が分析中</span>
          <span className="flex gap-1">
             <span className="w-1 h-1 bg-slate-100 rounded-full animate-bounce [animation-delay:-0.3s]"></span>
             <span className="w-1 h-1 bg-slate-100 rounded-full animate-bounce [animation-delay:-0.15s]"></span>
             <span className="w-1 h-1 bg-slate-100 rounded-full animate-bounce"></span>
          </span>
        </h2>
        ...
        <div className="h-12 flex items-start justify-center">
          <p 
            className={`text-xs font-bold text-slate-400 leading-relaxed transition-opacity duration-300 ${fade ? "opacity-100 translate-y-0" : "opacity-0 translate-y-2"}`}
          >
        ...
        <div className="w-48 h-1 bg-slate-800 rounded-full mx-auto overflow-hidden relative">
           <div className="absolute inset-0 bg-gradient-to-r from-blue-500 to-cyan-500 w-1/2 animate-[shimmer_1.5s_infinite_linear] rounded-full"></div>
        </div>
        <style jsx>{`
          @keyframes shimmer {
...
export default function CadenciaPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans pb-[420px] selection:bg-cyan-900 selection:text-white overflow-x-hidden">
      <style jsx global>{`
      ...
      {/* Header */}
      <header className="fixed top-0 inset-x-0 z-40 h-16 bg-slate-950/80 backdrop-blur-md border-b border-slate-800 flex items-center justify-between px-5 transition-all">
        <div className="flex items-center gap-3">
          {/* 修正: アイコン変更 */}
          <div className="w-8 h-8 flex items-center justify-center text-slate-100"><span className="text-xl">🎼</span></div>
          <div className="flex flex-col justify-center gap-0">
             {/* 修正: タイトルの色をheroTextStatic(slate-700)に合わせ、行間を詰める */}
             <div className="flex items-center gap-2"><span className="text-lg font-black tracking-tight text-slate-100 leading-none">Waon AI</span></div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col items-end justify-center">
             ...
             <div className="bg-slate-900 p-0.5 rounded-full flex border border-slate-800 shadow-inner gap-0.5">
               <button onClick={() => setMode("beginner")} className={`relative px-3 py-1.5 rounded-full text-[10px] font-bold flex items-center gap-1.5 transition-all duration-300 ${mode === "beginner" ? "bg-slate-800 text-emerald-400 shadow-sm ring-1 ring-emerald-900/50" : "text-slate-500 hover:bg-slate-800/50 hover:text-slate-400"}`}>
                 <span>🔰</span><span>初心者</span>
               </button>
               <button onClick={() => setMode("expert")} className={`relative px-3 py-1.5 rounded-full text-[10px] font-bold flex items-center gap-1.5 transition-all duration-300 ${mode === "expert" ? "bg-slate-800 text-blue-400 shadow-sm ring-1 ring-blue-900/50" : "text-slate-500 hover:bg-slate-800/50 hover:text-slate-400"}`}>
                 <span>🎓</span><span>上級者</span>
               </button>
             </div>
          </div>
          <span className="hidden sm:inline-block font-mono text-[10px] font-bold text-slate-400 border-l-2 border-slate-800 pl-3">v0.1.0</span>
        </div>
      </header>
      ...
      <main className="pt-20 px-5 max-w-md mx-auto space-y-6 relative z-10">
        ...
        <section className="text-center py-2 relative h-[100px] flex flex-col items-center justify-center">
            <div className="absolute top-2 left-8 text-4xl text-cyan-500/30 animate-float-1 pointer-events-none select-none">♪</div>
            <div className="absolute bottom-2 right-8 text-3xl text-blue-500/30 animate-float-2 pointer-events-none select-none">♫</div>
            <div className="absolute top-1/2 right-0 text-xl text-purple-500/30 animate-float-1 pointer-events-none select-none" style={{animationDelay: '1s'}}>♭</div>
           ...
           <div className="p-5 flex flex-col min-h-[240px] relative z-10">
              <div className="flex justify-between items-start mb-4">
                 <div className="space-y-0.5 pt-1">
                   ...
                   <p className="text-[10px] text-slate-500">キーボードをタップして音を追加</p>
                 </div>
                 
                 {/* ★ 修正: Keyと音数を表示する新しいステータスバッジ */}
                 <div className="flex items-stretch bg-slate-900 border border-slate-800 shadow-sm rounded-xl overflow-hidden divide-x divide-slate-800">
                    {/* Key表示エリア */}
                    <div className="px-3 py-1.5 flex flex-col items-center justify-center min-w-[64px]">
                       <span className="text-[8px] font-bold text-slate-500 leading-none mb-0.5 tracking-wide">KEY</span>
                       <span className={`text-xs font-black leading-none ${keyRoot !== "-" ? "text-purple-400" : "text-slate-600"}`}>
                          {keyRoot !== "-" ? (
                    ...
                    {/* 音数表示エリア */}
                    <div className="px-3 py-1.5 flex flex-col items-center justify-center min-w-[56px] bg-slate-800/50">
                       <span className="text-[8px] font-bold text-slate-500 leading-none mb-0.5 tracking-wide">NOTES</span>
                       <div className="flex items-baseline gap-0.5">
                          <span className={`text-lg font-black leading-none ${selected.length > 0 ? "text-cyan-400" : "text-slate-600"}`}>{selected.length}</span>
                          <span className="text-[9px] font-bold text-slate-500">音</span>
                       </div>
                    </div>
                 </div>
              </div>
                       ...
                       {sortedSelected.map((note) => (
                          ...
                          <div key={note} className={`relative group animate-in zoom-in duration-300 aspect-square`}>
                            ...
                            <div className="absolute -top-2 left-1/2 -translate-x-1/2 flex flex-col gap-1 items-center w-max pointer-events-none z-10">
                              ...
                              {bassHint === note && <span className="text-[8px] bg-amber-600 text-white px-1.5 py-0.5 rounded-full font-bold shadow-sm">最低音</span>}
                            </div>
                          </div>
                       ))}
                    </div>
        ...
        {hasResult && (
          <div ref={resultRef} className="space-y-6 animate-in fade-in slide-in-from-bottom-8 duration-700">
              <div className="flex items-center gap-2 px-1 py-2"><IconBook className="text-slate-100 w-5 h-5" /><h2 className="text-lg font-bold text-slate-100">Waon AIの分析結果 📖</h2></div>
              ...
              {candidates.length > 1 && (
                <div className="space-y-4">
                  {/* 修正: 「その他の可能性」に変更 */}
                  <div className="flex items-center justify-center py-4 gap-4"><div className="h-px bg-slate-800 flex-1"></div><span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">その他の可能性</span><div className="h-px bg-slate-800 flex-1"></div></div>
      ...
      <div className={`fixed bottom-0 inset-x-0 z-50 ${G.glassKeyContainer} rounded-t-[36px] transition-transform duration-300 ease-out touch-none ${isKeyboardOpen ? "translate-y-0" : "translate-y-[calc(100%-30px)]"}`} style={{ transform: isKeyboardOpen ? `translateY(${keyboardOffset}px)` : undefined }}>
        ...
        <div className="max-w-md mx-auto px-4 pb-8 pt-2">
          ...
          <div className="grid grid-cols-4 grid-rows-3 gap-2.5 h-full">
            ...
            <button className="col-start-4 row-start-1 h-16 rounded-2xl bg-slate-800/60 border border-slate-700 text-slate-500 active:text-rose-400 active:bg-rose-900/30 transition-all flex items-center justify-center shadow-sm active:scale-95 hover:bg-slate-800" onClick={reset}><IconTrash /></button>
            ...
            <div className="col-start-1 row-start-3 h-16 flex flex-col gap-1">
               <button onClick={() => setInputMode(m => m === "root" ? "normal" : "root")} className={`flex-1 rounded-t-xl rounded-b-sm text-[10px] font-bold transition-all border flex flex-col items-center justify-center leading-tight ${inputMode === "root" ? "bg-rose-600 text-white border-rose-500 shadow-inner" : "bg-slate-800/60 text-slate-400 border-slate-700 shadow-sm"}`}>根音指定</button>
               <button onClick={() => setInputMode(m => m === "bass" ? "normal" : "bass")} className={`flex-1 rounded-b-xl rounded-t-sm text-[10px] font-bold transition-all border flex flex-col items-center justify-center leading-tight ${inputMode === "bass" ? "bg-amber-600 text-white border-amber-500 shadow-inner" : "bg-slate-800/60 text-slate-400 border-slate-700 shadow-sm"}`}>最低音指定</button>
            </div>

            <div className="col-start-2 col-span-2 row-start-3 h-16 bg-slate-800/60 backdrop-blur-md rounded-2xl border border-slate-700 shadow-sm flex items-center overflow-hidden">
                <div className="flex-[0.8] flex items-center justify-center border-r border-dotted border-slate-600/30 h-full px-1"><span className="text-[10px] font-bold text-slate-400 whitespace-nowrap leading-tight text-center">調性は</span></div>
                <div className="flex-1 relative h-full border-r border-dotted border-slate-600/30 group active:bg-white/5 transition-colors">
                   {/* 修正: none -> - を反映 */}
                   ...
                   <div className="w-full h-full flex flex-col items-center justify-center pointer-events-none"><span className={`text-xs font-bold ${keyRoot === "-" ? "text-slate-500" : "text-cyan-400"}`}>{keyRoot === "-" ? "-" : keyRoot}</span></div>
                </div>
                {/* 修正: keyRoot判定を - に変更 */}
                <div className={`flex-1 relative h-full active:bg-white/5 transition-colors ${keyRoot === "-" ? "opacity-50" : ""}`}>
                   {/* 修正: Minor -> minor を反映 */}
                   ...
                   <div className="w-full h-full flex flex-col items-center justify-center pointer-events-none"><span className={`text-xs font-bold ${keyRoot === "-" ? "text-slate-600" : "text-purple-400"}`}>{keyType === "Major" ? "Major" : "minor"}</span></div>
                </div>
            </div>
            
            <button className={`col-start-4 row-start-3 h-16 rounded-2xl flex flex-col items-center justify-center shadow-lg transition-all active:scale-95 border border-white/10 relative overflow-hidden group ${canAnalyze && !loading ? "bg-cyan-600 text-white shadow-cyan-900/50" : "bg-slate-800 text-slate-600 cursor-not-allowed border-slate-700"}`} onClick={analyze} disabled={!canAnalyze || loading}>
